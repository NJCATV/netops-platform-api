...
def infrastructure_snapshot(force=False):
    ...
    # Host firewall rules require root access and are therefore labelled as
    # policy-unavailable; port/service reachability is still continuously proven.
    topology = [
        {"id": "web-entry", "from": "clients", "to": "233", "protocol": "HTTPS", "ports": "5772", "direction": "用户 → 平台", "description": "浏览器经 233:5772 访问统一网管入口（Nginx → Vue / BFF）", "status": node_status.get("233", "failed"), "firewall": "对外统一入口；端口与服务可达状态由平台探针验证"},
        {"id": "collector-api", "from": "233", "to": "236", "protocol": "HTTP", "ports": "18086", "direction": "233 → 236", "description": "采集探测、设备查询与采集状态", "status": link_status("233", "236"), "firewall": "端口/服务实测可达；策略未授权读取"},
        {"id": "collector-mysql", "from": "233", "to": "236", "protocol": "MySQL", "ports": "3339", "direction": "233 → 236", "description": "OLT、CMTS、ONU 与采集结果查询", "status": link_status("233", "236"), "firewall": "业务查询已验证；策略未授权读取"},
        {"id": "aiops-api", "from": "233", "to": "20", "protocol": "HTTP + 签名", "ports": "18080", "direction": "233 → 20", "description": "AIOps 分析、事件、日志与知识库代理", "status": link_status("233", "20"), "firewall": "服务健康已验证；策略未授权读取"},
        {"id": "clickhouse", "from": "233", "to": "212", "protocol": "ClickHouse HTTP", "ports": "8123", "direction": "233 → 212", "description": "ONU 质差、性能与 Radius 分析查询", "status": link_status("233", "212"), "firewall": "实时查询已验证；策略未授权读取"},
        {"id": "radius-udp", "from": "radius_nas", "to": "213", "protocol": "RADIUS / UDP", "ports": "1812 / 1813 / 3799", "direction": "NAS / BRAS → 213", "description": "认证、Accounting 与 CoA/Disconnect 报文镜像抓包", "status": node_status.get("213", "failed"), "firewall": "抓包服务运行已验证；策略未授权读取"},
        {"id": "radius-sink", "from": "213", "to": "212", "protocol": "ClickHouse HTTP", "ports": "8123", "direction": "213 → 212", "description": "解析结果与采集指标写入 Radius 分析库", "status": link_status("213", "212"), "firewall": "落库链路实时探测；策略未授权读取"},
    ]
...
def radius_accounting():
    ...
    top_users = radius_ch_query(
        f"""SELECT username,sum(input_delta) AS input_bytes,sum(output_delta) AS output_bytes,
                   sum(input_delta+output_delta) AS total_bytes,uniqExact(acct_session_id) AS sessions,
                   countIf(acct_status_type=1) AS starts,
                   countIf(counter_rollback=1) AS rollbacks,
                   round(input_bytes / greatest(output_bytes,1),2) AS upload_ratio
            FROM radius_events WHERE {where} AND username!=''
            GROUP BY username ORDER BY total_bytes DESC LIMIT 100 FORMAT JSON"""
    )
    terminate_causes = radius_ch_query(
        f"""SELECT terminate_cause,count() AS value
            FROM radius_events WHERE {where} AND acct_status_type=2
            GROUP BY terminate_cause ORDER BY value DESC LIMIT 12 FORMAT JSON"""
    )
    ...
    return success({"summary": summary, "traffic": traffic, "top_users": top_users,
                    "terminate_causes": terminate_causes, "quality": quality,
