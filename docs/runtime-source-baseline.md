...
- Canonical deployed NetOps route: `platform-adapter/host-application/backend/app/routes/netops2026.py`.
- The route is loaded by the `netops-littleProgram` Flask host application; it is not a standalone service.
...
- The companion navigation, verification and start scripts are stored under `platform-adapter/host-application/backend/`.
