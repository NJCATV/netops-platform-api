...
def choose_users():
    ...
    print(f"anbo_candidate_count={len(anbo)}")
    chosen = []
    predicates = (
...
def main() -> None:
    ...
    with app.app_context():
        ...
        for label, user in choose_users():
            ...
            assert ai_keys and window.get("syslog_parsed", 0) > 0
            ...
            management = user.role_code in ("org_admin", "super_admin")
            qq_status = client.get("/api/netops2026/aiops/system/qq-bot-status", headers=headers) if management else None
            qq_body = qq_status.get_json(silent=True) or {} if qq_status else {}
            ...
            print(
                ...
                f"{knowledge_data.get('topic_count')} qq={qq_status.status_code if qq_status else 'management-only'}/"
                f"{qq_data.get('status') or qq_data.get('online')}"
            )
            ...
            if management:
                assert qq_status is not None and qq_status.status_code == 200
                audit_statuses = {
